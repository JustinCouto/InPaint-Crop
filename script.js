...
const closeAiMenuBtn = document.getElementById('closeAiMenu');
...
/* =========================
  ...
  function showAiMenu() {
    ...
    // Clear replace textarea
    if (replaceTextarea) replaceTextarea.value = '';
    
    // Focus on menu for accessibility
    setTimeout(() => {
      if (closeAiMenuBtn) closeAiMenuBtn.focus();
    }, 100);
  }
  ...
  function validateReplaceText() {
    if (!replaceTextarea) return false;
    const text = replaceTextarea.value.trim();
    if (!text) {
      alert('Please describe what you want to replace the painted area with.');
...
function buildInpaintPrompt(mode = 'remove', replaceText = '') {
  const width = cropRect ? Math.round(cropRect.w) : displayW;
  const height = cropRect ? Math.round(cropRect.h) : displayH;

  if (mode === 'replace' && replaceText.trim()) {
    return `You are an expert image editing assistant.
  
  I will upload ONE file: combined-image.png (resolution: ${width}×${height} px).
  This image shows the original photo with SOLID RED (#ff0000) brush strokes drawn on top marking the area to be replaced.
  
  Task:
  - Replace ONLY the regions covered by the red strokes with: ${replaceText.trim()}
  - Seamlessly blend the new content with nearby context, maintaining consistent lighting, perspective, textures, and edges.
...
// AI Menu interactions
closeAiMenuBtn?.addEventListener('click', hideAiMenu);
...
window.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && aiMenu?.style.display === 'flex') {
    hideAiMenu();
  }
});

// AI Menu copy buttons
aiMenu?.addEventListener('click', async (e) => {
  const button = e.target.closest('.ai-icon-btn');
  if (!button) return;
  
  const action = button.dataset.action;
  if (!action) return;
  
  const [mode, type] = action.split('-'); // e.g., "remove-prompt" -> ["remove", "prompt"]
  
  // Validate replace mode
  if (mode === 'replace' && !validateReplaceText()) {
    return;
  }
  
  try {
    const replaceText = mode === 'replace' ? replaceTextarea?.value || '' : '';
    const promptText = buildInpaintPrompt(mode, replaceText);
    
    // Handle different copy types
    if (type === 'prompt') {
      await copyTextToClipboard(promptText);
      showButtonCopyFeedback(button);
    } else if (type === 'image') {
      if (!currentImageFile) {
      ...
      await copyForChatGPT(promptText, currentImageFile);
      showButtonCopyFeedback(button);
    }
  } catch (err) {
